  ...
  return (
    <nav className="wrapper">
      ...
      <Link to="/dict" className="DictNavLink" onClick={() => setActiveNav(1)}>
        <div>
          <div className={activeNav === 1 ? 'DictNavItem active' : 'DictNavItem'}>밈 단어</div>
          {/* 네비게이션을 구성하고 있는 하나의 버튼 */}
        </div>
      </Link>
      <Link to="/dict/question" className="DictNavLink" onClick={() => setActiveNav(2)}>
        <div>
          <div className={activeNav === 2 ? 'DictNavItem active' : 'DictNavItem'}>Q&A</div>
        </div>
      </Link>
      <Link to="/dict/stat" className="DictNavLink" onClick={() => setActiveNav(3)}>
        <div>
          <div className={activeNav === 3 ? 'DictNavItem active' : 'DictNavItem'}>통계</div>
        </div>
      </Link>
      <Link to="/dict/mymeme" className="DictNavLink" onClick={() => setActiveNav(4)}>
        <div>
          <div className={activeNav === 4 ? 'DictNavItem active' : 'DictNavItem'}>스크랩</div>
        </div>
      </Link>
    </nav>
  )
